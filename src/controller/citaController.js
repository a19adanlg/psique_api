...
const citaGET = async (req, res, next) => {
    try {
        ...
        if (cita) {
            ...
            return next(new CustomError("No existe la cita solicitada o no puedes acceder a ella", 404))
        }
    } catch (error) {
...
const citasPOST = async (req, res, next) => {
    try {
        ...
        if (token) {
            ...
            if (!nifPaciente)
                return next(new CustomError("Es necesario el NIF para solicitar una cita", 400));
        }

        // * Si no se especifica una especialidad, retornamos error
        if (!especialidad || (especialidad != "Psicología" && especialidad != "Psiquiatría"))
            return next(new CustomError("Es necesario que especifiques una especialidad válida para solicitar una cita", 400));
        ...
        if (isPaciente || !hasUsuario) {
            ...
            if (doctor) {
                ...
                return next(new CustomError("No existe ning\u00FAn doctor en esta especialidad", 404));
            }
        }

        if (!nifValido(nifPaciente))
            return next(new CustomError("El NIF del paciente no es v\u00E1lido", 400));
        if (!nifValido(nifDoctor))
            return next(new CustomError("El NIF del doctor no es v\u00E1lido", 400));
...
const citasPUT = async (req, res, next) => {
    try {
        ...
        if (cita) {
            ...
            if (usuario.rol == 'ROLE_DOCTOR') {
                if (nifDoctor) {
                    ...
                    if (existeDoctor) {
                        ...
                        return next(new CustomError("No existe el doctor al que reasignar la cita o no pertenece a la especialidad necesaria", 404))
                    }
                } else {
                ...
                return next(new CustomError("No tienes permisos para modificar esta cita", 403));
            }
            ...
            return next(new CustomError("No existe la cita solicitada", 404))
        }
    } catch (error) {
    ...
    try {
        ...
        if (cita) {
            ...
            if (usuario.rol == 'ROLE_DOCTOR') {
                ...
                return next(new CustomError("No tienes permisos para eliminar esta cita", 403));
            }
            ...
            if (delCita) {
                ...
                return next(new CustomError("No tienes permiso para eliminar esta cita", 403))
            }
        } else {
            return next(new CustomError("No existe la cita solicitada", 404))
        }
