...
const pacienteGET = async (req, res) => {
    try {
        ...
        if (paciente) {
            ...
            return next(new CustomError("Paciente solicitado no encontrado", 404));
        }
    } catch (error) {
...
const pacientesPOST = async (req, res, next) => {
    try {
        ...
        if (existePaciente)
            return next(new CustomError("Ya existe un paciente con ese NIF", 409));
        ...
        return next(new CustomError("Error inesperado en la petici\u00F3n POST", 409));
    }
};
...
const pacientesPUT = async (req, res, next) => {
    try {
        ...
        if (modPaciente) {
            ...
            return next(new CustomError("No existe el paciente solicitado", 404))
        }
    } catch (error) {
